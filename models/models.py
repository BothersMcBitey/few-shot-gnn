...
class MetricNN(nn.Module):
	...
	def gnn_iclr_forward(self, z, zi_s, labels_yi):
		...
		nodes = [lam(zi, label_yi) for zi, label_yi in zip(zi_s, labels_yi)]
...
def create_models(args):
	...
	if 'omniglot' == args.dataset:
		enc_nn = EmbeddingImagenet(args, 128) if True else EmbeddingOmniglot(args, 64)
	elif 'mini_imagenet' == args.dataset:
		enc_nn = EmbeddingImagenet(args, 128)#EmbeddingOmniglot(args, 64)#
	else:
